...
var Swipeable = function (_PureComponent) {
    ...
    function Swipeable() {
        ...
        return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = Swipeable.__proto__ || Object.getPrototypeOf(Swipeable)).call.apply(_ref, [this].concat(args))), _this), _this.state = {
            ...
            var resistentSignal = dx > 0 ? 1 : -1
            var resistentPct = 1.6 - (dx * resistentSignal) / _this.state.width
            // var positivePct = resistentPct < 1 ? resistentPct : 1
            // if (positivePct < 0)
            //     positivePct = positivePct * -1
            var resistentDx = dx * resistentPct

            // console.log('gestureState', dx, x, positivePct, isSwipingLeft, 'resistentDx', resistentDx)
            // console.log('isSwipingLeft', isSwipingLeft, dx, resistentDx, )
            // console.log('--', isSwipingLeft, isSwipingRight, dx)

            _this._handlePan(event, { dx: resistentDx, dy: gestureState.dy });
...
Swipeable.propTypes = {
    ...
    maxDistance: _propTypes.PropTypes.number,
